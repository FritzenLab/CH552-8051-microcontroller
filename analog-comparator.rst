                                      1 ;--------------------------------------------------------
                                      2 ; File Created by SDCC : free open source ISO C Compiler
                                      3 ; Version 4.5.0 #15242 (MINGW64)
                                      4 ;--------------------------------------------------------
                                      5 	.module analog_comparator
                                      6 	
                                      7 	.optsdcc -mmcs51 --model-small
                                      8 ;--------------------------------------------------------
                                      9 ; Public variables in this module
                                     10 ;--------------------------------------------------------
                                     11 	.globl _main
                                     12 	.globl _timer0_init
                                     13 	.globl _UIF_BUS_RST
                                     14 	.globl _UIF_DETECT
                                     15 	.globl _UIF_TRANSFER
                                     16 	.globl _UIF_SUSPEND
                                     17 	.globl _UIF_HST_SOF
                                     18 	.globl _UIF_FIFO_OV
                                     19 	.globl _U_SIE_FREE
                                     20 	.globl _U_TOG_OK
                                     21 	.globl _U_IS_NAK
                                     22 	.globl _ADC_CHAN0
                                     23 	.globl _ADC_CHAN1
                                     24 	.globl _CMP_CHAN
                                     25 	.globl _ADC_START
                                     26 	.globl _ADC_IF
                                     27 	.globl _CMP_IF
                                     28 	.globl _CMPO
                                     29 	.globl _U1RI
                                     30 	.globl _U1TI
                                     31 	.globl _U1RB8
                                     32 	.globl _U1TB8
                                     33 	.globl _U1REN
                                     34 	.globl _U1SMOD
                                     35 	.globl _U1SM0
                                     36 	.globl _S0_R_FIFO
                                     37 	.globl _S0_T_FIFO
                                     38 	.globl _S0_FREE
                                     39 	.globl _S0_IF_BYTE
                                     40 	.globl _S0_IF_FIRST
                                     41 	.globl _S0_IF_OV
                                     42 	.globl _S0_FST_ACT
                                     43 	.globl _CP_RL2
                                     44 	.globl _C_T2
                                     45 	.globl _TR2
                                     46 	.globl _EXEN2
                                     47 	.globl _TCLK
                                     48 	.globl _RCLK
                                     49 	.globl _EXF2
                                     50 	.globl _CAP1F
                                     51 	.globl _TF2
                                     52 	.globl _RI
                                     53 	.globl _TI
                                     54 	.globl _RB8
                                     55 	.globl _TB8
                                     56 	.globl _REN
                                     57 	.globl _SM2
                                     58 	.globl _SM1
                                     59 	.globl _SM0
                                     60 	.globl _IT0
                                     61 	.globl _IE0
                                     62 	.globl _IT1
                                     63 	.globl _IE1
                                     64 	.globl _TR0
                                     65 	.globl _TF0
                                     66 	.globl _TR1
                                     67 	.globl _TF1
                                     68 	.globl _RXD
                                     69 	.globl _PWM1_
                                     70 	.globl _TXD
                                     71 	.globl _PWM2_
                                     72 	.globl _AIN3
                                     73 	.globl _VBUS1
                                     74 	.globl _INT0
                                     75 	.globl _TXD1_
                                     76 	.globl _INT1
                                     77 	.globl _T0
                                     78 	.globl _RXD1_
                                     79 	.globl _PWM2
                                     80 	.globl _T1
                                     81 	.globl _UDP
                                     82 	.globl _UDM
                                     83 	.globl _TIN0
                                     84 	.globl _CAP1
                                     85 	.globl _T2
                                     86 	.globl _AIN0
                                     87 	.globl _VBUS2
                                     88 	.globl _TIN1
                                     89 	.globl _CAP2
                                     90 	.globl _T2EX
                                     91 	.globl _RXD_
                                     92 	.globl _TXD_
                                     93 	.globl _AIN1
                                     94 	.globl _UCC1
                                     95 	.globl _TIN2
                                     96 	.globl _SCS
                                     97 	.globl _CAP1_
                                     98 	.globl _T2_
                                     99 	.globl _AIN2
                                    100 	.globl _UCC2
                                    101 	.globl _TIN3
                                    102 	.globl _PWM1
                                    103 	.globl _MOSI
                                    104 	.globl _TIN4
                                    105 	.globl _RXD1
                                    106 	.globl _MISO
                                    107 	.globl _TIN5
                                    108 	.globl _TXD1
                                    109 	.globl _SCK
                                    110 	.globl _IE_SPI0
                                    111 	.globl _IE_TKEY
                                    112 	.globl _IE_USB
                                    113 	.globl _IE_ADC
                                    114 	.globl _IE_UART1
                                    115 	.globl _IE_PWMX
                                    116 	.globl _IE_GPIO
                                    117 	.globl _IE_WDOG
                                    118 	.globl _PX0
                                    119 	.globl _PT0
                                    120 	.globl _PX1
                                    121 	.globl _PT1
                                    122 	.globl _PS
                                    123 	.globl _PT2
                                    124 	.globl _PL_FLAG
                                    125 	.globl _PH_FLAG
                                    126 	.globl _EX0
                                    127 	.globl _ET0
                                    128 	.globl _EX1
                                    129 	.globl _ET1
                                    130 	.globl _ES
                                    131 	.globl _ET2
                                    132 	.globl _E_DIS
                                    133 	.globl _EA
                                    134 	.globl _P
                                    135 	.globl _F1
                                    136 	.globl _OV
                                    137 	.globl _RS0
                                    138 	.globl _RS1
                                    139 	.globl _F0
                                    140 	.globl _AC
                                    141 	.globl _CY
                                    142 	.globl _UEP1_DMA_H
                                    143 	.globl _UEP1_DMA_L
                                    144 	.globl _UEP1_DMA
                                    145 	.globl _UEP0_DMA_H
                                    146 	.globl _UEP0_DMA_L
                                    147 	.globl _UEP0_DMA
                                    148 	.globl _UEP2_3_MOD
                                    149 	.globl _UEP4_1_MOD
                                    150 	.globl _UEP3_DMA_H
                                    151 	.globl _UEP3_DMA_L
                                    152 	.globl _UEP3_DMA
                                    153 	.globl _UEP2_DMA_H
                                    154 	.globl _UEP2_DMA_L
                                    155 	.globl _UEP2_DMA
                                    156 	.globl _USB_DEV_AD
                                    157 	.globl _USB_CTRL
                                    158 	.globl _USB_INT_EN
                                    159 	.globl _UEP4_T_LEN
                                    160 	.globl _UEP4_CTRL
                                    161 	.globl _UEP0_T_LEN
                                    162 	.globl _UEP0_CTRL
                                    163 	.globl _USB_RX_LEN
                                    164 	.globl _USB_MIS_ST
                                    165 	.globl _USB_INT_ST
                                    166 	.globl _USB_INT_FG
                                    167 	.globl _UEP3_T_LEN
                                    168 	.globl _UEP3_CTRL
                                    169 	.globl _UEP2_T_LEN
                                    170 	.globl _UEP2_CTRL
                                    171 	.globl _UEP1_T_LEN
                                    172 	.globl _UEP1_CTRL
                                    173 	.globl _UDEV_CTRL
                                    174 	.globl _USB_C_CTRL
                                    175 	.globl _TKEY_DATH
                                    176 	.globl _TKEY_DATL
                                    177 	.globl _TKEY_DAT
                                    178 	.globl _TKEY_CTRL
                                    179 	.globl _ADC_DATA
                                    180 	.globl _ADC_CFG
                                    181 	.globl _ADC_CTRL
                                    182 	.globl _SBAUD1
                                    183 	.globl _SBUF1
                                    184 	.globl _SCON1
                                    185 	.globl _SPI0_SETUP
                                    186 	.globl _SPI0_CK_SE
                                    187 	.globl _SPI0_CTRL
                                    188 	.globl _SPI0_DATA
                                    189 	.globl _SPI0_STAT
                                    190 	.globl _PWM_CK_SE
                                    191 	.globl _PWM_CTRL
                                    192 	.globl _PWM_DATA1
                                    193 	.globl _PWM_DATA2
                                    194 	.globl _T2CAP1H
                                    195 	.globl _T2CAP1L
                                    196 	.globl _T2CAP1
                                    197 	.globl _TH2
                                    198 	.globl _TL2
                                    199 	.globl _T2COUNT
                                    200 	.globl _RCAP2H
                                    201 	.globl _RCAP2L
                                    202 	.globl _RCAP2
                                    203 	.globl _T2MOD
                                    204 	.globl _T2CON
                                    205 	.globl _SBUF
                                    206 	.globl _SCON
                                    207 	.globl _TH1
                                    208 	.globl _TH0
                                    209 	.globl _TL1
                                    210 	.globl _TL0
                                    211 	.globl _TMOD
                                    212 	.globl _TCON
                                    213 	.globl _XBUS_AUX
                                    214 	.globl _PIN_FUNC
                                    215 	.globl _P3_DIR_PU
                                    216 	.globl _P3_MOD_OC
                                    217 	.globl _P3
                                    218 	.globl _P2
                                    219 	.globl _P1_DIR_PU
                                    220 	.globl _P1_MOD_OC
                                    221 	.globl _P1
                                    222 	.globl _ROM_CTRL
                                    223 	.globl _ROM_DATA_H
                                    224 	.globl _ROM_DATA_L
                                    225 	.globl _ROM_DATA
                                    226 	.globl _ROM_ADDR_H
                                    227 	.globl _ROM_ADDR_L
                                    228 	.globl _ROM_ADDR
                                    229 	.globl _GPIO_IE
                                    230 	.globl _IP_EX
                                    231 	.globl _IE_EX
                                    232 	.globl _IP
                                    233 	.globl _IE
                                    234 	.globl _WDOG_COUNT
                                    235 	.globl _RESET_KEEP
                                    236 	.globl _WAKE_CTRL
                                    237 	.globl _CLOCK_CFG
                                    238 	.globl _PCON
                                    239 	.globl _GLOBAL_CFG
                                    240 	.globl _SAFE_MOD
                                    241 	.globl _DPH
                                    242 	.globl _DPL
                                    243 	.globl _SP
                                    244 	.globl _B
                                    245 	.globl _ACC
                                    246 	.globl _PSW
                                    247 	.globl _ledON
                                    248 	.globl _t
                                    249 	.globl _counter
                                    250 	.globl _serialTime
                                    251 	.globl _comparatorTime
                                    252 	.globl _tick_10ms
                                    253 	.globl _clock_init
                                    254 	.globl _timer0_ISR
                                    255 	.globl _cmp_init
                                    256 	.globl _blink_led
                                    257 ;--------------------------------------------------------
                                    258 ; special function registers
                                    259 ;--------------------------------------------------------
                                    260 	.area RSEG    (ABS,DATA)
      000000                        261 	.org 0x0000
                           0000D0   262 _PSW	=	0x00d0
                           0000E0   263 _ACC	=	0x00e0
                           0000F0   264 _B	=	0x00f0
                           000081   265 _SP	=	0x0081
                           000082   266 _DPL	=	0x0082
                           000083   267 _DPH	=	0x0083
                           0000A1   268 _SAFE_MOD	=	0x00a1
                           0000B1   269 _GLOBAL_CFG	=	0x00b1
                           000087   270 _PCON	=	0x0087
                           0000B9   271 _CLOCK_CFG	=	0x00b9
                           0000A9   272 _WAKE_CTRL	=	0x00a9
                           0000FE   273 _RESET_KEEP	=	0x00fe
                           0000FF   274 _WDOG_COUNT	=	0x00ff
                           0000A8   275 _IE	=	0x00a8
                           0000B8   276 _IP	=	0x00b8
                           0000E8   277 _IE_EX	=	0x00e8
                           0000E9   278 _IP_EX	=	0x00e9
                           0000C7   279 _GPIO_IE	=	0x00c7
                           008584   280 _ROM_ADDR	=	0x8584
                           000084   281 _ROM_ADDR_L	=	0x0084
                           000085   282 _ROM_ADDR_H	=	0x0085
                           008F8E   283 _ROM_DATA	=	0x8f8e
                           00008E   284 _ROM_DATA_L	=	0x008e
                           00008F   285 _ROM_DATA_H	=	0x008f
                           000086   286 _ROM_CTRL	=	0x0086
                           000090   287 _P1	=	0x0090
                           000092   288 _P1_MOD_OC	=	0x0092
                           000093   289 _P1_DIR_PU	=	0x0093
                           0000A0   290 _P2	=	0x00a0
                           0000B0   291 _P3	=	0x00b0
                           000096   292 _P3_MOD_OC	=	0x0096
                           000097   293 _P3_DIR_PU	=	0x0097
                           0000C6   294 _PIN_FUNC	=	0x00c6
                           0000A2   295 _XBUS_AUX	=	0x00a2
                           000088   296 _TCON	=	0x0088
                           000089   297 _TMOD	=	0x0089
                           00008A   298 _TL0	=	0x008a
                           00008B   299 _TL1	=	0x008b
                           00008C   300 _TH0	=	0x008c
                           00008D   301 _TH1	=	0x008d
                           000098   302 _SCON	=	0x0098
                           000099   303 _SBUF	=	0x0099
                           0000C8   304 _T2CON	=	0x00c8
                           0000C9   305 _T2MOD	=	0x00c9
                           00CBCA   306 _RCAP2	=	0xcbca
                           0000CA   307 _RCAP2L	=	0x00ca
                           0000CB   308 _RCAP2H	=	0x00cb
                           00CDCC   309 _T2COUNT	=	0xcdcc
                           0000CC   310 _TL2	=	0x00cc
                           0000CD   311 _TH2	=	0x00cd
                           00CFCE   312 _T2CAP1	=	0xcfce
                           0000CE   313 _T2CAP1L	=	0x00ce
                           0000CF   314 _T2CAP1H	=	0x00cf
                           00009B   315 _PWM_DATA2	=	0x009b
                           00009C   316 _PWM_DATA1	=	0x009c
                           00009D   317 _PWM_CTRL	=	0x009d
                           00009E   318 _PWM_CK_SE	=	0x009e
                           0000F8   319 _SPI0_STAT	=	0x00f8
                           0000F9   320 _SPI0_DATA	=	0x00f9
                           0000FA   321 _SPI0_CTRL	=	0x00fa
                           0000FB   322 _SPI0_CK_SE	=	0x00fb
                           0000FC   323 _SPI0_SETUP	=	0x00fc
                           0000C0   324 _SCON1	=	0x00c0
                           0000C1   325 _SBUF1	=	0x00c1
                           0000C2   326 _SBAUD1	=	0x00c2
                           000080   327 _ADC_CTRL	=	0x0080
                           00009A   328 _ADC_CFG	=	0x009a
                           00009F   329 _ADC_DATA	=	0x009f
                           0000C3   330 _TKEY_CTRL	=	0x00c3
                           00C5C4   331 _TKEY_DAT	=	0xc5c4
                           0000C4   332 _TKEY_DATL	=	0x00c4
                           0000C5   333 _TKEY_DATH	=	0x00c5
                           000091   334 _USB_C_CTRL	=	0x0091
                           0000D1   335 _UDEV_CTRL	=	0x00d1
                           0000D2   336 _UEP1_CTRL	=	0x00d2
                           0000D3   337 _UEP1_T_LEN	=	0x00d3
                           0000D4   338 _UEP2_CTRL	=	0x00d4
                           0000D5   339 _UEP2_T_LEN	=	0x00d5
                           0000D6   340 _UEP3_CTRL	=	0x00d6
                           0000D7   341 _UEP3_T_LEN	=	0x00d7
                           0000D8   342 _USB_INT_FG	=	0x00d8
                           0000D9   343 _USB_INT_ST	=	0x00d9
                           0000DA   344 _USB_MIS_ST	=	0x00da
                           0000DB   345 _USB_RX_LEN	=	0x00db
                           0000DC   346 _UEP0_CTRL	=	0x00dc
                           0000DD   347 _UEP0_T_LEN	=	0x00dd
                           0000DE   348 _UEP4_CTRL	=	0x00de
                           0000DF   349 _UEP4_T_LEN	=	0x00df
                           0000E1   350 _USB_INT_EN	=	0x00e1
                           0000E2   351 _USB_CTRL	=	0x00e2
                           0000E3   352 _USB_DEV_AD	=	0x00e3
                           00E5E4   353 _UEP2_DMA	=	0xe5e4
                           0000E4   354 _UEP2_DMA_L	=	0x00e4
                           0000E5   355 _UEP2_DMA_H	=	0x00e5
                           00E7E6   356 _UEP3_DMA	=	0xe7e6
                           0000E6   357 _UEP3_DMA_L	=	0x00e6
                           0000E7   358 _UEP3_DMA_H	=	0x00e7
                           0000EA   359 _UEP4_1_MOD	=	0x00ea
                           0000EB   360 _UEP2_3_MOD	=	0x00eb
                           00EDEC   361 _UEP0_DMA	=	0xedec
                           0000EC   362 _UEP0_DMA_L	=	0x00ec
                           0000ED   363 _UEP0_DMA_H	=	0x00ed
                           00EFEE   364 _UEP1_DMA	=	0xefee
                           0000EE   365 _UEP1_DMA_L	=	0x00ee
                           0000EF   366 _UEP1_DMA_H	=	0x00ef
                                    367 ;--------------------------------------------------------
                                    368 ; special function bits
                                    369 ;--------------------------------------------------------
                                    370 	.area RSEG    (ABS,DATA)
      000000                        371 	.org 0x0000
                           0000D7   372 _CY	=	0x00d7
                           0000D6   373 _AC	=	0x00d6
                           0000D5   374 _F0	=	0x00d5
                           0000D4   375 _RS1	=	0x00d4
                           0000D3   376 _RS0	=	0x00d3
                           0000D2   377 _OV	=	0x00d2
                           0000D1   378 _F1	=	0x00d1
                           0000D0   379 _P	=	0x00d0
                           0000AF   380 _EA	=	0x00af
                           0000AE   381 _E_DIS	=	0x00ae
                           0000AD   382 _ET2	=	0x00ad
                           0000AC   383 _ES	=	0x00ac
                           0000AB   384 _ET1	=	0x00ab
                           0000AA   385 _EX1	=	0x00aa
                           0000A9   386 _ET0	=	0x00a9
                           0000A8   387 _EX0	=	0x00a8
                           0000BF   388 _PH_FLAG	=	0x00bf
                           0000BE   389 _PL_FLAG	=	0x00be
                           0000BD   390 _PT2	=	0x00bd
                           0000BC   391 _PS	=	0x00bc
                           0000BB   392 _PT1	=	0x00bb
                           0000BA   393 _PX1	=	0x00ba
                           0000B9   394 _PT0	=	0x00b9
                           0000B8   395 _PX0	=	0x00b8
                           0000EF   396 _IE_WDOG	=	0x00ef
                           0000EE   397 _IE_GPIO	=	0x00ee
                           0000ED   398 _IE_PWMX	=	0x00ed
                           0000EC   399 _IE_UART1	=	0x00ec
                           0000EB   400 _IE_ADC	=	0x00eb
                           0000EA   401 _IE_USB	=	0x00ea
                           0000E9   402 _IE_TKEY	=	0x00e9
                           0000E8   403 _IE_SPI0	=	0x00e8
                           000097   404 _SCK	=	0x0097
                           000097   405 _TXD1	=	0x0097
                           000097   406 _TIN5	=	0x0097
                           000096   407 _MISO	=	0x0096
                           000096   408 _RXD1	=	0x0096
                           000096   409 _TIN4	=	0x0096
                           000095   410 _MOSI	=	0x0095
                           000095   411 _PWM1	=	0x0095
                           000095   412 _TIN3	=	0x0095
                           000095   413 _UCC2	=	0x0095
                           000095   414 _AIN2	=	0x0095
                           000094   415 _T2_	=	0x0094
                           000094   416 _CAP1_	=	0x0094
                           000094   417 _SCS	=	0x0094
                           000094   418 _TIN2	=	0x0094
                           000094   419 _UCC1	=	0x0094
                           000094   420 _AIN1	=	0x0094
                           000093   421 _TXD_	=	0x0093
                           000092   422 _RXD_	=	0x0092
                           000091   423 _T2EX	=	0x0091
                           000091   424 _CAP2	=	0x0091
                           000091   425 _TIN1	=	0x0091
                           000091   426 _VBUS2	=	0x0091
                           000091   427 _AIN0	=	0x0091
                           000090   428 _T2	=	0x0090
                           000090   429 _CAP1	=	0x0090
                           000090   430 _TIN0	=	0x0090
                           0000B7   431 _UDM	=	0x00b7
                           0000B6   432 _UDP	=	0x00b6
                           0000B5   433 _T1	=	0x00b5
                           0000B4   434 _PWM2	=	0x00b4
                           0000B4   435 _RXD1_	=	0x00b4
                           0000B4   436 _T0	=	0x00b4
                           0000B3   437 _INT1	=	0x00b3
                           0000B2   438 _TXD1_	=	0x00b2
                           0000B2   439 _INT0	=	0x00b2
                           0000B2   440 _VBUS1	=	0x00b2
                           0000B2   441 _AIN3	=	0x00b2
                           0000B1   442 _PWM2_	=	0x00b1
                           0000B1   443 _TXD	=	0x00b1
                           0000B0   444 _PWM1_	=	0x00b0
                           0000B0   445 _RXD	=	0x00b0
                           00008F   446 _TF1	=	0x008f
                           00008E   447 _TR1	=	0x008e
                           00008D   448 _TF0	=	0x008d
                           00008C   449 _TR0	=	0x008c
                           00008B   450 _IE1	=	0x008b
                           00008A   451 _IT1	=	0x008a
                           000089   452 _IE0	=	0x0089
                           000088   453 _IT0	=	0x0088
                           00009F   454 _SM0	=	0x009f
                           00009E   455 _SM1	=	0x009e
                           00009D   456 _SM2	=	0x009d
                           00009C   457 _REN	=	0x009c
                           00009B   458 _TB8	=	0x009b
                           00009A   459 _RB8	=	0x009a
                           000099   460 _TI	=	0x0099
                           000098   461 _RI	=	0x0098
                           0000CF   462 _TF2	=	0x00cf
                           0000CF   463 _CAP1F	=	0x00cf
                           0000CE   464 _EXF2	=	0x00ce
                           0000CD   465 _RCLK	=	0x00cd
                           0000CC   466 _TCLK	=	0x00cc
                           0000CB   467 _EXEN2	=	0x00cb
                           0000CA   468 _TR2	=	0x00ca
                           0000C9   469 _C_T2	=	0x00c9
                           0000C8   470 _CP_RL2	=	0x00c8
                           0000FF   471 _S0_FST_ACT	=	0x00ff
                           0000FE   472 _S0_IF_OV	=	0x00fe
                           0000FD   473 _S0_IF_FIRST	=	0x00fd
                           0000FC   474 _S0_IF_BYTE	=	0x00fc
                           0000FB   475 _S0_FREE	=	0x00fb
                           0000FA   476 _S0_T_FIFO	=	0x00fa
                           0000F8   477 _S0_R_FIFO	=	0x00f8
                           0000C7   478 _U1SM0	=	0x00c7
                           0000C5   479 _U1SMOD	=	0x00c5
                           0000C4   480 _U1REN	=	0x00c4
                           0000C3   481 _U1TB8	=	0x00c3
                           0000C2   482 _U1RB8	=	0x00c2
                           0000C1   483 _U1TI	=	0x00c1
                           0000C0   484 _U1RI	=	0x00c0
                           000087   485 _CMPO	=	0x0087
                           000086   486 _CMP_IF	=	0x0086
                           000085   487 _ADC_IF	=	0x0085
                           000084   488 _ADC_START	=	0x0084
                           000083   489 _CMP_CHAN	=	0x0083
                           000081   490 _ADC_CHAN1	=	0x0081
                           000080   491 _ADC_CHAN0	=	0x0080
                           0000DF   492 _U_IS_NAK	=	0x00df
                           0000DE   493 _U_TOG_OK	=	0x00de
                           0000DD   494 _U_SIE_FREE	=	0x00dd
                           0000DC   495 _UIF_FIFO_OV	=	0x00dc
                           0000DB   496 _UIF_HST_SOF	=	0x00db
                           0000DA   497 _UIF_SUSPEND	=	0x00da
                           0000D9   498 _UIF_TRANSFER	=	0x00d9
                           0000D8   499 _UIF_DETECT	=	0x00d8
                           0000D8   500 _UIF_BUS_RST	=	0x00d8
                                    501 ;--------------------------------------------------------
                                    502 ; overlayable register banks
                                    503 ;--------------------------------------------------------
                                    504 	.area REG_BANK_0	(REL,OVR,DATA)
      000000                        505 	.ds 8
                                    506 	.area REG_BANK_1	(REL,OVR,DATA)
      000008                        507 	.ds 8
                                    508 ;--------------------------------------------------------
                                    509 ; internal ram data
                                    510 ;--------------------------------------------------------
                                    511 	.area DSEG    (DATA)
      000010                        512 _tick_10ms::
      000010                        513 	.ds 2
      000012                        514 _comparatorTime::
      000012                        515 	.ds 2
      000014                        516 _serialTime::
      000014                        517 	.ds 2
      000016                        518 _counter::
      000016                        519 	.ds 2
      000018                        520 _t::
      000018                        521 	.ds 2
      00001A                        522 _blink_base:
      00001A                        523 	.ds 2
                                    524 ;--------------------------------------------------------
                                    525 ; overlayable items in internal ram
                                    526 ;--------------------------------------------------------
                                    527 ;--------------------------------------------------------
                                    528 ; Stack segment in internal ram
                                    529 ;--------------------------------------------------------
                                    530 	.area SSEG
      000021                        531 __start__stack:
      000021                        532 	.ds	1
                                    533 
                                    534 ;--------------------------------------------------------
                                    535 ; indirectly addressable internal ram data
                                    536 ;--------------------------------------------------------
                                    537 	.area ISEG    (DATA)
                                    538 ;--------------------------------------------------------
                                    539 ; absolute internal ram data
                                    540 ;--------------------------------------------------------
                                    541 	.area IABS    (ABS,DATA)
                                    542 	.area IABS    (ABS,DATA)
                                    543 ;--------------------------------------------------------
                                    544 ; bit data
                                    545 ;--------------------------------------------------------
                                    546 	.area BSEG    (BIT)
      000000                        547 _ledON::
      000000                        548 	.ds 1
      000001                        549 _wdt_started:
      000001                        550 	.ds 1
                                    551 ;--------------------------------------------------------
                                    552 ; paged external ram data
                                    553 ;--------------------------------------------------------
                                    554 	.area PSEG    (PAG,XDATA)
                                    555 ;--------------------------------------------------------
                                    556 ; uninitialized external ram data
                                    557 ;--------------------------------------------------------
                                    558 	.area XSEG    (XDATA)
                                    559 ;--------------------------------------------------------
                                    560 ; absolute external ram data
                                    561 ;--------------------------------------------------------
                                    562 	.area XABS    (ABS,XDATA)
                                    563 ;--------------------------------------------------------
                                    564 ; initialized external ram data
                                    565 ;--------------------------------------------------------
                                    566 	.area XISEG   (XDATA)
                                    567 	.area HOME    (CODE)
                                    568 	.area GSINIT0 (CODE)
                                    569 	.area GSINIT1 (CODE)
                                    570 	.area GSINIT2 (CODE)
                                    571 	.area GSINIT3 (CODE)
                                    572 	.area GSINIT4 (CODE)
                                    573 	.area GSINIT5 (CODE)
                                    574 	.area GSINIT  (CODE)
                                    575 	.area GSFINAL (CODE)
                                    576 	.area CSEG    (CODE)
                                    577 ;--------------------------------------------------------
                                    578 ; interrupt vector
                                    579 ;--------------------------------------------------------
                                    580 	.area HOME    (CODE)
      000000                        581 __interrupt_vect:
      000000 02 00 54         [24]  582 	ljmp	__sdcc_gsinit_startup
      000003 32               [24]  583 	reti
      000004                        584 	.ds	7
      00000B 02 00 E1         [24]  585 	ljmp	_timer0_ISR
                                    586 ; restartable atomic support routines
      00000E                        587 	.ds	2
      000010                        588 sdcc_atomic_exchange_rollback_start::
      000010 00               [12]  589 	nop
      000011 00               [12]  590 	nop
      000012                        591 sdcc_atomic_exchange_pdata_impl:
      000012 E2               [24]  592 	movx	a, @r0
      000013 FB               [12]  593 	mov	r3, a
      000014 EA               [12]  594 	mov	a, r2
      000015 F2               [24]  595 	movx	@r0, a
      000016 80 2C            [24]  596 	sjmp	sdcc_atomic_exchange_exit
      000018 00               [12]  597 	nop
      000019 00               [12]  598 	nop
      00001A                        599 sdcc_atomic_exchange_xdata_impl:
      00001A E0               [24]  600 	movx	a, @dptr
      00001B FB               [12]  601 	mov	r3, a
      00001C EA               [12]  602 	mov	a, r2
      00001D F0               [24]  603 	movx	@dptr, a
      00001E 80 24            [24]  604 	sjmp	sdcc_atomic_exchange_exit
      000020                        605 sdcc_atomic_compare_exchange_idata_impl:
      000020 E6               [12]  606 	mov	a, @r0
      000021 B5 02 02         [24]  607 	cjne	a, ar2, .+#5
      000024 EB               [12]  608 	mov	a, r3
      000025 F6               [12]  609 	mov	@r0, a
      000026 22               [24]  610 	ret
      000027 00               [12]  611 	nop
      000028                        612 sdcc_atomic_compare_exchange_pdata_impl:
      000028 E2               [24]  613 	movx	a, @r0
      000029 B5 02 02         [24]  614 	cjne	a, ar2, .+#5
      00002C EB               [12]  615 	mov	a, r3
      00002D F2               [24]  616 	movx	@r0, a
      00002E 22               [24]  617 	ret
      00002F 00               [12]  618 	nop
      000030                        619 sdcc_atomic_compare_exchange_xdata_impl:
      000030 E0               [24]  620 	movx	a, @dptr
      000031 B5 02 02         [24]  621 	cjne	a, ar2, .+#5
      000034 EB               [12]  622 	mov	a, r3
      000035 F0               [24]  623 	movx	@dptr, a
      000036 22               [24]  624 	ret
      000037                        625 sdcc_atomic_exchange_rollback_end::
                                    626 
      000037                        627 sdcc_atomic_exchange_gptr_impl::
      000037 30 F6 E0         [24]  628 	jnb	b.6, sdcc_atomic_exchange_xdata_impl
      00003A A8 82            [24]  629 	mov	r0, dpl
      00003C 20 F5 D3         [24]  630 	jb	b.5, sdcc_atomic_exchange_pdata_impl
      00003F                        631 sdcc_atomic_exchange_idata_impl:
      00003F EA               [12]  632 	mov	a, r2
      000040 C6               [12]  633 	xch	a, @r0
      000041 F5 82            [12]  634 	mov	dpl, a
      000043 22               [24]  635 	ret
      000044                        636 sdcc_atomic_exchange_exit:
      000044 8B 82            [24]  637 	mov	dpl, r3
      000046 22               [24]  638 	ret
      000047                        639 sdcc_atomic_compare_exchange_gptr_impl::
      000047 30 F6 E6         [24]  640 	jnb	b.6, sdcc_atomic_compare_exchange_xdata_impl
      00004A A8 82            [24]  641 	mov	r0, dpl
      00004C 20 F5 D9         [24]  642 	jb	b.5, sdcc_atomic_compare_exchange_pdata_impl
      00004F 80 CF            [24]  643 	sjmp	sdcc_atomic_compare_exchange_idata_impl
                                    644 ;--------------------------------------------------------
                                    645 ; global & static initialisations
                                    646 ;--------------------------------------------------------
                                    647 	.area HOME    (CODE)
                                    648 	.area GSINIT  (CODE)
                                    649 	.area GSFINAL (CODE)
                                    650 	.area GSINIT  (CODE)
                                    651 	.globl __sdcc_gsinit_startup
                                    652 	.globl __sdcc_program_startup
                                    653 	.globl __start__stack
                                    654 	.globl __mcs51_genXINIT
                                    655 	.globl __mcs51_genXRAMCLEAR
                                    656 	.globl __mcs51_genRAMCLEAR
                                    657 ;	analog-comparator.c:5: volatile unsigned int tick_10ms = 0;
      0000AD E4               [12]  658 	clr	a
      0000AE F5 10            [12]  659 	mov	_tick_10ms,a
      0000B0 F5 11            [12]  660 	mov	(_tick_10ms + 1),a
                                    661 ;	analog-comparator.c:6: volatile unsigned int comparatorTime = 0;
      0000B2 F5 12            [12]  662 	mov	_comparatorTime,a
      0000B4 F5 13            [12]  663 	mov	(_comparatorTime + 1),a
                                    664 ;	analog-comparator.c:8: unsigned int serialTime= 0;
      0000B6 F5 14            [12]  665 	mov	_serialTime,a
      0000B8 F5 15            [12]  666 	mov	(_serialTime + 1),a
                                    667 ;	analog-comparator.c:9: unsigned int counter= 0;
      0000BA F5 16            [12]  668 	mov	_counter,a
      0000BC F5 17            [12]  669 	mov	(_counter + 1),a
                                    670 ;	analog-comparator.c:12: static unsigned int blink_base = 0;
      0000BE F5 1A            [12]  671 	mov	_blink_base,a
      0000C0 F5 1B            [12]  672 	mov	(_blink_base + 1),a
                                    673 ;	analog-comparator.c:7: volatile __bit ledON = 0;
                                    674 ;	assignBit
      0000C2 C2 00            [12]  675 	clr	_ledON
                                    676 ;	analog-comparator.c:11: static __bit wdt_started = 0;
                                    677 ;	assignBit
      0000C4 C2 01            [12]  678 	clr	_wdt_started
                                    679 	.area GSFINAL (CODE)
      0000C9 02 00 51         [24]  680 	ljmp	__sdcc_program_startup
                                    681 ;--------------------------------------------------------
                                    682 ; Home
                                    683 ;--------------------------------------------------------
                                    684 	.area HOME    (CODE)
                                    685 	.area HOME    (CODE)
      000051                        686 __sdcc_program_startup:
      000051 02 01 5C         [24]  687 	ljmp	_main
                                    688 ;	return from main will return to caller
                                    689 ;--------------------------------------------------------
                                    690 ; code
                                    691 ;--------------------------------------------------------
                                    692 	.area CSEG    (CODE)
                                    693 ;------------------------------------------------------------
                                    694 ;Allocation info for local variables in function 'clock_init'
                                    695 ;------------------------------------------------------------
                                    696 ;	analog-comparator.c:19: void clock_init(void) {
                                    697 ;	-----------------------------------------
                                    698 ;	 function clock_init
                                    699 ;	-----------------------------------------
      0000CC                        700 _clock_init:
                           000007   701 	ar7 = 0x07
                           000006   702 	ar6 = 0x06
                           000005   703 	ar5 = 0x05
                           000004   704 	ar4 = 0x04
                           000003   705 	ar3 = 0x03
                           000002   706 	ar2 = 0x02
                           000001   707 	ar1 = 0x01
                           000000   708 	ar0 = 0x00
                                    709 ;	analog-comparator.c:20: SAFE_MOD = 0x55;
      0000CC 75 A1 55         [24]  710 	mov	_SAFE_MOD,#0x55
                                    711 ;	analog-comparator.c:21: SAFE_MOD = 0xAA;
      0000CF 75 A1 AA         [24]  712 	mov	_SAFE_MOD,#0xaa
                                    713 ;	analog-comparator.c:22: CLOCK_CFG |= bOSC_EN_INT; 
      0000D2 43 B9 80         [24]  714 	orl	_CLOCK_CFG,#0x80
                                    715 ;	analog-comparator.c:24: CLOCK_CFG = (CLOCK_CFG & ~MASK_SYS_CK_SEL) | 0x06;
      0000D5 74 F8            [12]  716 	mov	a,#0xf8
      0000D7 55 B9            [12]  717 	anl	a,_CLOCK_CFG
      0000D9 44 06            [12]  718 	orl	a,#0x06
      0000DB F5 B9            [12]  719 	mov	_CLOCK_CFG,a
                                    720 ;	analog-comparator.c:26: SAFE_MOD = 0x00;
      0000DD 75 A1 00         [24]  721 	mov	_SAFE_MOD,#0x00
                                    722 ;	analog-comparator.c:27: }
      0000E0 22               [24]  723 	ret
                                    724 ;------------------------------------------------------------
                                    725 ;Allocation info for local variables in function 'timer0_ISR'
                                    726 ;------------------------------------------------------------
                                    727 ;	analog-comparator.c:29: void timer0_ISR(void) __interrupt(1) __using(1){ 
                                    728 ;	-----------------------------------------
                                    729 ;	 function timer0_ISR
                                    730 ;	-----------------------------------------
      0000E1                        731 _timer0_ISR:
                           00000F   732 	ar7 = 0x0f
                           00000E   733 	ar6 = 0x0e
                           00000D   734 	ar5 = 0x0d
                           00000C   735 	ar4 = 0x0c
                           00000B   736 	ar3 = 0x0b
                           00000A   737 	ar2 = 0x0a
                           000009   738 	ar1 = 0x09
                           000008   739 	ar0 = 0x08
      0000E1 C0 E0            [24]  740 	push	acc
      0000E3 C0 D0            [24]  741 	push	psw
      0000E5 75 D0 08         [24]  742 	mov	psw,#0x08
                                    743 ;	analog-comparator.c:31: TF0 = 0;  // clear overflow flag (important for robustness)
                                    744 ;	assignBit
      0000E8 C2 8D            [12]  745 	clr	_TF0
                                    746 ;	analog-comparator.c:32: TH0 = 0xB1;
      0000EA 75 8C B1         [24]  747 	mov	_TH0,#0xb1
                                    748 ;	analog-comparator.c:33: TL0 = 0xE0;
      0000ED 75 8A E0         [24]  749 	mov	_TL0,#0xe0
                                    750 ;	analog-comparator.c:34: tick_10ms++; // this is the 10ms tick for LED blinking
      0000F0 AE 10            [24]  751 	mov	r6,_tick_10ms
      0000F2 AF 11            [24]  752 	mov	r7,(_tick_10ms + 1)
      0000F4 74 01            [12]  753 	mov	a,#0x01
      0000F6 2E               [12]  754 	add	a, r6
      0000F7 F5 10            [12]  755 	mov	_tick_10ms,a
      0000F9 E4               [12]  756 	clr	a
      0000FA 3F               [12]  757 	addc	a, r7
      0000FB F5 11            [12]  758 	mov	(_tick_10ms + 1),a
                                    759 ;	analog-comparator.c:36: }
      0000FD D0 D0            [24]  760 	pop	psw
      0000FF D0 E0            [24]  761 	pop	acc
      000101 32               [24]  762 	reti
                                    763 ;	eliminated unneeded push/pop dpl
                                    764 ;	eliminated unneeded push/pop dph
                                    765 ;	eliminated unneeded push/pop b
                                    766 ;------------------------------------------------------------
                                    767 ;Allocation info for local variables in function 'timer0_init'
                                    768 ;------------------------------------------------------------
                                    769 ;	analog-comparator.c:38: void timer0_init(void) {
                                    770 ;	-----------------------------------------
                                    771 ;	 function timer0_init
                                    772 ;	-----------------------------------------
      000102                        773 _timer0_init:
                           000007   774 	ar7 = 0x07
                           000006   775 	ar6 = 0x06
                           000005   776 	ar5 = 0x05
                           000004   777 	ar4 = 0x04
                           000003   778 	ar3 = 0x03
                           000002   779 	ar2 = 0x02
                           000001   780 	ar1 = 0x01
                           000000   781 	ar0 = 0x00
                                    782 ;	analog-comparator.c:40: T2MOD &= ~bTMR_CLK;   // disable fast clock mode
      000102 53 C9 7F         [24]  783 	anl	_T2MOD,#0x7f
                                    784 ;	analog-comparator.c:41: T2MOD &= ~bT0_CLK;    // Timer0 = Fsys/12
      000105 53 C9 EF         [24]  785 	anl	_T2MOD,#0xef
                                    786 ;	analog-comparator.c:42: TMOD &= ~0x03;  // clear Timer0 mode bits
      000108 53 89 FC         [24]  787 	anl	_TMOD,#0xfc
                                    788 ;	analog-comparator.c:43: TMOD |=  0x01;  // Timer0 mode 1: 16-bit
      00010B 43 89 01         [24]  789 	orl	_TMOD,#0x01
                                    790 ;	analog-comparator.c:46: TH0 = 0xB1;
      00010E 75 8C B1         [24]  791 	mov	_TH0,#0xb1
                                    792 ;	analog-comparator.c:47: TL0 = 0xE0;
      000111 75 8A E0         [24]  793 	mov	_TL0,#0xe0
                                    794 ;	analog-comparator.c:49: TF0 = 0;
                                    795 ;	assignBit
      000114 C2 8D            [12]  796 	clr	_TF0
                                    797 ;	analog-comparator.c:51: ET0 = 1;   // enable Timer0 interrupt
                                    798 ;	assignBit
      000116 D2 A9            [12]  799 	setb	_ET0
                                    800 ;	analog-comparator.c:52: TR0 = 1;   // start Timer0
                                    801 ;	assignBit
      000118 D2 8C            [12]  802 	setb	_TR0
                                    803 ;	analog-comparator.c:53: EA = 1;
                                    804 ;	assignBit
      00011A D2 AF            [12]  805 	setb	_EA
                                    806 ;	analog-comparator.c:54: }
      00011C 22               [24]  807 	ret
                                    808 ;------------------------------------------------------------
                                    809 ;Allocation info for local variables in function 'cmp_init'
                                    810 ;------------------------------------------------------------
                                    811 ;	analog-comparator.c:55: void cmp_init(void) {
                                    812 ;	-----------------------------------------
                                    813 ;	 function cmp_init
                                    814 ;	-----------------------------------------
      00011D                        815 _cmp_init:
                                    816 ;	analog-comparator.c:57: ADC_CFG = bCMP_EN;
      00011D 75 9A 04         [24]  817 	mov	_ADC_CFG,#0x04
                                    818 ;	analog-comparator.c:62: ADC_CTRL = 0x00;
      000120 75 80 00         [24]  819 	mov	_ADC_CTRL,#0x00
                                    820 ;	analog-comparator.c:65: P1_MOD_OC &= ~(1 << 1);
      000123 53 92 FD         [24]  821 	anl	_P1_MOD_OC,#0xfd
                                    822 ;	analog-comparator.c:66: P1_DIR_PU &= ~(1 << 1);
      000126 53 93 FD         [24]  823 	anl	_P1_DIR_PU,#0xfd
                                    824 ;	analog-comparator.c:69: P1_MOD_OC &= ~(1 << 4);
      000129 53 92 EF         [24]  825 	anl	_P1_MOD_OC,#0xef
                                    826 ;	analog-comparator.c:70: P1_DIR_PU &= ~(1 << 4);
      00012C 53 93 EF         [24]  827 	anl	_P1_DIR_PU,#0xef
                                    828 ;	analog-comparator.c:71: }
      00012F 22               [24]  829 	ret
                                    830 ;------------------------------------------------------------
                                    831 ;Allocation info for local variables in function 'blink_led'
                                    832 ;------------------------------------------------------------
                                    833 ;t             Allocated to registers r6 r7 
                                    834 ;phase         Allocated to registers r4 r5 
                                    835 ;------------------------------------------------------------
                                    836 ;	analog-comparator.c:72: void blink_led(unsigned int t) {
                                    837 ;	-----------------------------------------
                                    838 ;	 function blink_led
                                    839 ;	-----------------------------------------
      000130                        840 _blink_led:
      000130 AE 82            [24]  841 	mov	r6, dpl
      000132 AF 83            [24]  842 	mov	r7, dph
                                    843 ;	analog-comparator.c:73: unsigned int phase = t - blink_base;
      000134 EE               [12]  844 	mov	a,r6
      000135 C3               [12]  845 	clr	c
      000136 95 1A            [12]  846 	subb	a,_blink_base
      000138 FC               [12]  847 	mov	r4,a
      000139 EF               [12]  848 	mov	a,r7
      00013A 95 1B            [12]  849 	subb	a,(_blink_base + 1)
      00013C FD               [12]  850 	mov	r5,a
                                    851 ;	analog-comparator.c:75: if (phase < 15) {
      00013D C3               [12]  852 	clr	c
      00013E EC               [12]  853 	mov	a,r4
      00013F 94 0F            [12]  854 	subb	a,#0x0f
      000141 ED               [12]  855 	mov	a,r5
      000142 94 00            [12]  856 	subb	a,#0x00
      000144 50 04            [24]  857 	jnc	00105$
                                    858 ;	analog-comparator.c:76: P3 |= (1 << 0);
      000146 43 B0 01         [24]  859 	orl	_P3,#0x01
      000149 22               [24]  860 	ret
      00014A                        861 00105$:
                                    862 ;	analog-comparator.c:77: } else if (phase < 30) {
      00014A C3               [12]  863 	clr	c
      00014B EC               [12]  864 	mov	a,r4
      00014C 94 1E            [12]  865 	subb	a,#0x1e
      00014E ED               [12]  866 	mov	a,r5
      00014F 94 00            [12]  867 	subb	a,#0x00
      000151 50 04            [24]  868 	jnc	00102$
                                    869 ;	analog-comparator.c:78: P3 &= ~(1 << 0);
      000153 53 B0 FE         [24]  870 	anl	_P3,#0xfe
      000156 22               [24]  871 	ret
      000157                        872 00102$:
                                    873 ;	analog-comparator.c:80: blink_base = t;         
      000157 8E 1A            [24]  874 	mov	_blink_base,r6
      000159 8F 1B            [24]  875 	mov	(_blink_base + 1),r7
                                    876 ;	analog-comparator.c:82: }
      00015B 22               [24]  877 	ret
                                    878 ;------------------------------------------------------------
                                    879 ;Allocation info for local variables in function 'main'
                                    880 ;------------------------------------------------------------
                                    881 ;	analog-comparator.c:84: void main(void) {
                                    882 ;	-----------------------------------------
                                    883 ;	 function main
                                    884 ;	-----------------------------------------
      00015C                        885 _main:
                                    886 ;	analog-comparator.c:85: clock_init();    
      00015C 12 00 CC         [24]  887 	lcall	_clock_init
                                    888 ;	analog-comparator.c:86: timer0_init();  
      00015F 12 01 02         [24]  889 	lcall	_timer0_init
                                    890 ;	analog-comparator.c:87: cmp_init();
      000162 12 01 1D         [24]  891 	lcall	_cmp_init
                                    892 ;	analog-comparator.c:91: P3_MOD_OC &= ~(1 << 0);   // push-pull
      000165 53 96 FE         [24]  893 	anl	_P3_MOD_OC,#0xfe
                                    894 ;	analog-comparator.c:92: P3_DIR_PU |=  (1 << 0);   // enable strong output drive
      000168 43 97 01         [24]  895 	orl	_P3_DIR_PU,#0x01
                                    896 ;	analog-comparator.c:94: P3 &= ~(1 << 0);  // Make LED pin P3.0 "start" as OFF
      00016B 53 B0 FE         [24]  897 	anl	_P3,#0xfe
                                    898 ;	analog-comparator.c:96: while (1) {        
      00016E                        899 00111$:
                                    900 ;	analog-comparator.c:98: EA = 0;
                                    901 ;	assignBit
      00016E C2 AF            [12]  902 	clr	_EA
                                    903 ;	analog-comparator.c:99: t = tick_10ms;
      000170 85 10 18         [24]  904 	mov	_t,_tick_10ms
      000173 85 11 19         [24]  905 	mov	(_t + 1),(_tick_10ms + 1)
                                    906 ;	analog-comparator.c:100: EA = 1;
                                    907 ;	assignBit
      000176 D2 AF            [12]  908 	setb	_EA
                                    909 ;	analog-comparator.c:102: if(t - comparatorTime > 10){
      000178 E5 18            [12]  910 	mov	a,_t
      00017A C3               [12]  911 	clr	c
      00017B 95 12            [12]  912 	subb	a,_comparatorTime
      00017D FE               [12]  913 	mov	r6,a
      00017E E5 19            [12]  914 	mov	a,(_t + 1)
      000180 95 13            [12]  915 	subb	a,(_comparatorTime + 1)
      000182 FF               [12]  916 	mov	r7,a
      000183 C3               [12]  917 	clr	c
      000184 74 0A            [12]  918 	mov	a,#0x0a
      000186 9E               [12]  919 	subb	a,r6
      000187 E4               [12]  920 	clr	a
      000188 9F               [12]  921 	subb	a,r7
      000189 50 1B            [24]  922 	jnc	00107$
                                    923 ;	analog-comparator.c:103: comparatorTime= t;
      00018B 85 18 12         [24]  924 	mov	_comparatorTime,_t
      00018E 85 19 13         [24]  925 	mov	(_comparatorTime + 1),(_t + 1)
                                    926 ;	analog-comparator.c:105: if (CMPO) { // CMP0 does not latch and does not generate interrupts, it is just a mirror of the analog comparator
      000191 30 87 0D         [24]  927 	jnb	_CMPO,00104$
                                    928 ;	analog-comparator.c:106: if (!ledON) {
      000194 20 00 0F         [24]  929 	jb	_ledON,00107$
                                    930 ;	analog-comparator.c:107: ledON = 1;
                                    931 ;	assignBit
      000197 D2 00            [12]  932 	setb	_ledON
                                    933 ;	analog-comparator.c:108: blink_base = t;  // reset blink phase when comparator first triggers
      000199 85 18 1A         [24]  934 	mov	_blink_base,_t
      00019C 85 19 1B         [24]  935 	mov	(_blink_base + 1),(_t + 1)
      00019F 80 05            [24]  936 	sjmp	00107$
      0001A1                        937 00104$:
                                    938 ;	analog-comparator.c:111: ledON = 0;
                                    939 ;	assignBit
      0001A1 C2 00            [12]  940 	clr	_ledON
                                    941 ;	analog-comparator.c:112: P3 &= ~(1 << 0);  // make sure LED goes off immediately when pot drops below reference
      0001A3 53 B0 FE         [24]  942 	anl	_P3,#0xfe
      0001A6                        943 00107$:
                                    944 ;	analog-comparator.c:117: if (ledON) {
      0001A6 30 00 C5         [24]  945 	jnb	_ledON,00111$
                                    946 ;	analog-comparator.c:118: blink_led(t);   // called every loop iteration while CMPO is high
      0001A9 85 18 82         [24]  947 	mov	dpl, _t
      0001AC 85 19 83         [24]  948 	mov	dph, (_t + 1)
      0001AF 12 01 30         [24]  949 	lcall	_blink_led
                                    950 ;	analog-comparator.c:121: }
      0001B2 80 BA            [24]  951 	sjmp	00111$
                                    952 	.area CSEG    (CODE)
                                    953 	.area CONST   (CODE)
                                    954 	.area XINIT   (CODE)
                                    955 	.area CABS    (ABS,CODE)
